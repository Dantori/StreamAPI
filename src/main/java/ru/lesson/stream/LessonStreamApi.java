...
public class LessonStreamApi {
    ...
    public List<String> task2(List<Employee> employees) {
        return employees.stream()
                .filter((i) -> i.getRating() < 50)
                .distinct()
                .map(i -> i.getName() + "=" + i.getRating())
                .collect(Collectors.toList());
    }
    ...
    public List<Employee> task4(List<List<Employee>> employeeDepartments) {
        return employeeDepartments.stream()
                .flatMap(List::stream)
                .distinct()
                .sorted((employee1, employee2) -> Integer.compare(employee2.getRating(), employee1.getRating()))
                .collect(Collectors.toList());
    }
    ...
    public boolean task7(List<Employee> employees) {
        long countOriginal = employees.stream()
                .map(Employee::getName)
                .count();
        long countDistinct = employees.stream()
                .map(Employee::getName)
                .distinct()
                .count();
